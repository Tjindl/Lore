        ...
        const startCmd =
          process.platform === 'darwin'
            ...
            : process.platform === 'win32'
              ? 'cmd'
              : 'xdg-open';
        const startArgs = process.platform === 'win32' ? ['/c', 'start', url] : [url];
        execFile(startCmd, startArgs, (err) => {
          if (err) {
            console.log(
              chalk.dim(`  (Could not open browser automatically. Please visit ${url} manually)`)
            );
          }
        });
      }
