...
if [ "\\\${LINECOUNT:-0}" -gt 50 ]; then
  # Only prompt if we have an interactive TTY and /dev/tty is available
  if [ -t 0 ] && [ -t 1 ] && [ -c /dev/tty ]; then
    ...
    # Non-interactive commit detected, skipping prompt
    exit 0
  fi
fi
`;
      ...
      if (hasSource) {
        ...
        try {
          ...
          if (count > 0) {
            console.log(
              chalk.cyan(
                `  Found ${count} draft entr${count === 1 ? 'y' : 'ies'} — run: lore drafts`
              )
            );
          }
        } catch (e) {
