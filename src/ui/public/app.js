document.addEventListener('DOMContentLoaded', () => {
    ...
    entries.forEach((entry) => {
      ...
      card.innerHTML = `
                <div class="entry-header">
                    ...
                    <span class="entry-date muted">${escapeHtml((entry.date || 'Unknown').split('T')[0])}</span>
                </div>
  ...
  document.getElementById('kb-search').addEventListener('input', (e) => {
    ...
    const filtered = allEntries.filter((e) => {
      const matchQuery =
        e.title.toLowerCase().includes(query) || e.context.toLowerCase().includes(query);
  ...
  async function loadGraph() {
    ...
    try {
      ...
      document.getElementById('network-container').innerHTML =
        '<p class="muted" style="padding: 20px;">Could not render graph data.</p>';
    }
  }
